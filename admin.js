...
function setupTabs() {
  ...
  const allTabs = ['products', 'collections', 'orders', 'topProducts', 'slider', 'settings', 'quiz', 'coupons', 'reviews', 'abandonedCarts', 'customers'];
  tabBtns.forEach(btn => {
...
function renderCouponsList() {
  ...
  wrap.innerHTML = allCoupons.map(c => {
    const valueLabel = c.type === 'percent' ? `${c.value}%` : `${c.value} ${CURRENCIES[0]}`;
    ...
    return `<div class="bg-white rounded-2xl border border-gray-200 p-4 flex items-center justify-between flex-wrap gap-3">
      <div>
        ...
        <p class="text-xs text-gray-500">الخصم: <span class="font-bold text-gray-700">${valueLabel}</span> &nbsp;|&nbsp; مرات الاستخدام: <span class="font-bold text-gray-700">${usageLabel}</span></p>
      </div>
...
async function handleCouponFormSubmit(e) {
  ...
  try {
    ...
    const data = { code, type, value, usageLimit, active, updatedAt: serverTimestamp() };
    if (docId) {
...
function renderQuizAnswerBreakdown(data) {
  ...
  const sections = [
    ...
    { key: 'intensity', label: '🔥 أكتر ثبات مطلوب' },
